...
test.group('Services', () => {
  test('Create Service', async ({ client, assert }) => {
    const response = await client.post('http://api.zalgow.xyz/api/users/register').json(user)
    ...
    const responseService = await client
      .post('http://api.zalgow.xyz/api/restaurants/' + idRestaurant + '/services')
      .header('Authorization', `Bearer ${userToken}`)
      .json(service)
  ...
  test('Edit my service', async ({ client, assert }) => {
    const responseService = await client
      .put('http://api.zalgow.xyz/api/restaurants/' + idRestaurant + '/services/' + idService)
      .header('Authorization', `Bearer ${userToken}`)
      .json(serviceEdit)
    responseService.assertStatus(200)
